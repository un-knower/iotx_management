...
public class IotxServiceImpl implements IotxService{
	...
	/***
	 * 基本思路是count groupBy province,city
	 * 查看结果是否为1,为1表示都是同一个行政区划的,那么找下一级
	 * @param predicate
	 */
	@Override
	public JSONArray ascertainArea(Predicate predicate){
		...
		long countProvince = queryFactory.from(qIotxCustom).where(predicate).groupBy(qIotxCustom.district.city.province).fetchCount();
		if(countProvince==1){
			//如果都是一个省，那么查看是否都是一个市
			long countCity = queryFactory.from(qIotx).where(predicate).groupBy(qIotx.district.city).fetchCount();
			if(countCity==1){
				//如果都是一个市，那么就按照区来统计数据
				iotxTuples = queryFactory.select(qIotx.district.city.name,qIotx.count()).from(qIotx).where(predicate).groupBy(qIotx.district.city).fetch();
			}else{
				//如果是多个市，就按照市统计
				iotxTuples = queryFactory.select(qIotx.district.name,qIotx.count()).from(qIotx).where(predicate).groupBy(qIotx.district).fetch();
			}
		}else{
			//如果是多个省，就按照省统计
			iotxTuples = queryFactory.select(qIotxCustom.district.city.province.name,qIotx.count()).from(qIotx).where(predicate).groupBy(qIotxCustom.district.city.province).fetch();
		}
	...
	/****
	 * 貌似遇到个bug，对象链超过三个后就为null了
	 * 比如qIotx.district.city.province结果为null
